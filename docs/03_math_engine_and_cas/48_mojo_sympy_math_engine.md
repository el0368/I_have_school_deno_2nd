...
The `math_engine/` folder is a server-side math microservice powered by **Mojo**
(calling **SymPy** via Python interop). It handles PhD-level symbolic math that
cannot run in the browser via WebAssembly.

## Why This Stack

- **Mojo**: Compiled, fast, modern language that can import Python libraries
  directly
- **SymPy**: The gold standard open-source CAS, covering Grade 1 through
  PhD-level math
- **Why not WASM?**: SymPy depends on Python. Mojo/Python cannot compile to
  `wasm32-unknown-unknown`
- **Why not Rust/mathcore for this?**: `mathcore` covers basic algebra but lacks
  calculus, equation solving, limits, series, matrices, and step-by-step
  heuristics
...
Run these commands inside your WSL terminal. The Pixi environment MUST live in
the Linux home directory (`~`), NOT on the mounted Windows drive (`/mnt/c/`),
because NTFS does not support Linux file permissions.
...
The `source ~/.bashrc` is required because non-interactive WSL sessions do not
load the PATH automatically.
...
Always copy `pixi.toml` and `pixi.lock` back to Windows so they are committed to
Git.
...
The Mojo server will receive the equation, pass it to SymPy, and return LaTeX
back to Deno.

## Production Deployment

On a Linux VPS or Docker container, WSL is not needed. The full project runs
natively:
